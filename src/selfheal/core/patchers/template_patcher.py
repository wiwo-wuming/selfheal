    ...
    def _generate_fallback_patch(self, classification: ClassificationEvent) -> tuple[str, Optional[str]]:
        ...
        patches: dict[str, str] = {
            ...
            "type": (
                ...
                f"@@ -{error_line},1 +{error_line},1 @@\n"
                f"-{original_code}\n"
                f"+str({original_code})  # SelfHeal: convert to expected type\n"
            ),
            "value": (
                f"--- a/{target_file}\n"
                f"+++ b/{target_file}\n"
                f"@@ -{error_line},1 +{error_line},1 @@\n"
                f"-{original_code}\n"
                f"+{original_code.strip() if original_code.strip() and not original_code.startswith('#') else 'pass'}  # SelfHeal: guard value with sensible default\n"
            ),
            "syntax": (
                f"--- a/{target_file}\n"
                f"+++ b/{target_file}\n"
                f"@@ -{error_line},1 +{error_line},3 @@\n"
                f"-{original_code}\n"
                f"+# SelfHeal: syntax fix required — manual review needed\n"
                f"+# Original error: {event.error_message}\n"
            ),
            "runtime": (
                f"--- a/{target_file}\n"
                f"+++ b/{target_file}\n"
            ...
            "timeout": (
                ...
                f"@@ -{error_line},1 +{error_line},4 @@\n"
                f"-{original_code}\n"
                f"+# SelfHeal: added timeout handling\n"
                f"+import signal\n"
                f"+signal.alarm(30)\n"
                f"+{original_code.strip() if original_code.strip() and not original_code.startswith('#') else 'pass  # (original logic)'}\n"
            ),
            "network": (
                ...
                f"-{original_code}\n"
                f"+# SelfHeal: retry on network error with backoff\n"
                f"+import time\n"
                f"+for _retry in range(3):\n"
                f"+    try:\n"
                f"+        {original_code.strip() if original_code.strip() and not original_code.startswith('#') else 'pass  # (original logic)'}\n"
                f"+        break\n"
                f"+    except {event.error_type}:\n"
                f"+        time.sleep(2 ** _retry)\n"
            ),
            "config": (
                f"--- a/{target_file}\n"
                f"+++ b/{target_file}\n"
                f"@@ -{error_line},1 +{error_line},6 @@\n"
                f"-{original_code}\n"
                f"+# SelfHeal: use environment variable with sensible fallback\n"
                f"+import os\n"
                f"+_val = os.environ.get(\"SELFHEAL_REQUIRED_CONFIG\", \"default\")\n"
                f"+{original_code.strip() if original_code.strip() and not original_code.startswith('#') else 'pass  # (use _val)'}\n"
            ),
            "dependency": (
                f"--- a/{target_file}\n"
                f"+++ b/{target_file}\n"
                f"@@ -{error_line},1 +{error_line},6 @@\n"
                f"-{original_code}\n"
                f"+# SelfHeal: try-except for optional dependency\n"
                f"+try:\n"
                f"+    {original_code.strip() if original_code.strip() and not original_code.startswith('#') else 'pass  # (original logic)'}\n"
                f"+except ImportError:\n"
                f"+    pass  # optional dependency not available\n"
            ),
            "resource": (
                f"--- a/{target_file}\n"
                f"+++ b/{target_file}\n"
                f"@@ -{error_line},1 +{error_line},6 @@\n"
                f"-{original_code}\n"
                f"+# SelfHeal: check resource availability\n"
                f"+from pathlib import Path\n"
                f"+if Path(\"{target_file}\").exists():\n"
                f"+    {original_code.strip() if original_code.strip() and not original_code.startswith('#') else 'pass  # (original logic)'}\n"
            ),
            "permission": (
                f"--- a/{target_file}\n"
                ...
                f"@@ -{error_line},1 +{error_line},6 @@\n"
                f"-{original_code}\n"
                f"+# SelfHeal: wrap in try-except for permission errors\n"
                f"+try:\n"
                f"+    {original_code.strip() if original_code.strip() and not original_code.startswith('#') else 'pass  # (original logic)'}\n"
                f"+except PermissionError:\n"
                f"+    pass  # insufficient permissions\n"
            ),
            "flaky": (
                f"--- a/{target_file}\n"
                f"+++ b/{target_file}\n"
                ...
                f"-{original_code}\n"
                f"+# SelfHeal: added retry for flaky test\n"
                f"+import pytest\n"
            ...
            "memory": (
                ...
                f"@@ -{error_line},1 +{error_line},6 @@\n"
                f"-{original_code}\n"
                f"+# SelfHeal: convert to streaming to avoid MemoryError\n"
                f"+from itertools import islice\n"
                f"+_batch_size = 1000\n"
                f"+{original_code.strip() if original_code.strip() and not original_code.startswith('#') else 'pass  # (use _batch_size)'}\n"
            ),
        }
        ...
        content = patches.get(
            ...
            (
                ...
                f"@@ -{error_line},1 +{error_line},7 @@\n"
                f"-{original_code}\n"
                f"+# SelfHeal: guarded with try/except for {event.error_type}\n"
                f"+try:\n"
                f"+    {original_code.strip() if original_code.strip() and not original_code.startswith('#') else 'pass  # (original logic)'}\n"
                f"+except {event.error_type}:\n"
                f"+    pass  # SelfHeal: error handled\n"
            ),
        )
